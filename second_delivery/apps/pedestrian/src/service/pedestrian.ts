...
export async function GetPedestrians(): Promise<Pedestrian[]> {
    var response: AxiosResponse = await axios.get('http://localhost:3003/pedestrian');
...
export async function PostPedestrian(pedestrian: Pedestrian): Promise<Pedestrian> {
    var response: AxiosResponse = await axios.post('http://localhost:3003/pedestrian', pedestrian);
    var pedestrian: Pedestrian = response.data;
    return pedestrian;
}

export async function EditPedestrian(pedestrian: Pedestrian): Promise<Pedestrian> {
    var response: AxiosResponse = await axios.put('http://localhost:3003/pedestrian/' + pedestrian.getId(), pedestrian);
    var pedestrian: Pedestrian = response.data;
    return pedestrian;
}

export async function DeletePedestrian(id: Number): Promise<Pedestrian> {
    var response: AxiosResponse = await axios.delete('http://localhost:3003/pedestrian/' + id);
    var pedestrian: Pedestrian = response.data;
    return pedestrian;
}